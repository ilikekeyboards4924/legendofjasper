...
const camera: Camera = new Camera(renderer);

const logo: TexturedRect = new TexturedRect(0, 0, 960, 540, logoImage);
const titleCard: TexturedRect = new TexturedRect(0, 0, 960, 540, titleImage);
const button: StartButton = new StartButton(renderer.canvas.width - 128 - 40, 40, 128, 64, startButtonAnimation);
const player: Player = new Player(0, 0, 96*2/3, 243*2/3, playerTextures);

let decors: Decor[] = [];
for (let i = 0; i < 100; i++) {
    let decor: Decor = new Decor(random(-renderer.canvas.width*5, renderer.canvas.width*6), random(-renderer.canvas.height*5, renderer.canvas.height*6), 102, 105, signDecorImage);
    decors.push(decor);
}
